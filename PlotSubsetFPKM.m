...
elseif type == 'd'
clustergram(log2(FPKM(ismember(Gname,B),:)+0.1),'DisplayRatio',[0.1 0.1],'DisplayRange',12,'RowLabels',Gname(ismember(Gname,B)),'RowPDist','correlation','ColumnLabels',head,'Linkage','complete','Colormap',colormap(jet),'Symmetric','false')
elseif type == 'h'
...
elseif type == 'oc'
clustergram(log2(FPKM(GetOrder(flipud(B),Gname),:)+0.1),'OptimalLeafOrder','true','Cluster',2,'Linkage','complete','DisplayRatio',[0.1 0.1],'DisplayRange',12,'RowLabels',Gname(GetOrder(flipud(B),Gname)),'ColumnLabels',head,'Colormap',colormap(jet),'Symmetric','false')
end
